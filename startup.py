...
st.markdown("<h1 style = 'color: #DAC0A3; text-align: center; font-family: helvetica'>STARTUP PROFIT PREDICTOR</h1>", unsafe_allow_html = True)
st.markdown("<h4 style = 'margin: -30px; color: #F11A7B; text-align: center; font-family: Monaco  '>Built By Gomycode Data Science Daintree</h4>", unsafe_allow_html = True)
...
st.markdown("<br>", unsafe_allow_html = True)
#display the users input variables
...
#transform the users input wit the imported scalers
...
#st.success(f'The predicted profit for your organistration is: {predicted[0].round(2)}') to predict

# #if st.button('predict profit'):
#     st.balloons()
#     st.success(f'The predicted profit for your organistration is: {predicted[0].round(2)}')
...
with inter:
    ...
    coef = model.coef_
    inter.write('A percentage increase in Research and development expense makes profit to increase by {coef[0]}.naira')
